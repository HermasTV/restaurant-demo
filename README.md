...
Requires Python 3.12, a CUDA-capable GPU (tested on RTX 4070 Laptop with
CUDA 13 wheels), and [`uv`](https://docs.astral.sh/uv/) for env + package
management.

### 1. Install `uv` (skip if already installed)

```bash
# macOS / Linux (one-liner — installs to ~/.local/bin)
curl -LsSf https://astral.sh/uv/install.sh | sh
# Windows (PowerShell)
powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
```

Reopen your shell or `source ~/.bashrc` / `~/.zshrc` so `uv` is on PATH.
...
source task/bin/activate              # Windows: task\Scripts\activate
```

This creates `./task/` (gitignored). All commands below assume the venv
is active.

### 3. Install PyTorch with CUDA

`requirements.txt` doesn't pin torch — install it explicitly so `uv`
picks the right CUDA wheel for your driver. CUDA 12.x is the safest
default; switch the index URL if you need a different toolkit:
...
This pulls Ultralytics (YOLO26 + YOLOE), boxmot (BoT-SORT + ReID),
MiVOLO, Streamlit, OpenCV, supervision, and the streamlit-drawable
component.

### 5. Get the model weights

The pipeline expects four weight files under `data/weights/` (the
folder is gitignored so they don't bloat the repo):

| File | Source | Used by |
|---|---|---|
| `yolo26l.pt` | auto-downloaded by Ultralytics on first run if missing | person detector |
| `osnet_x1_0_msmt17.pt` | auto-downloaded by boxmot to its `models/` cache | BoT-SORT ReID |
| `yoloe-11s-seg.pt` | auto-downloaded by Ultralytics | PPE backbone |
| `yoloe_vpe_kitchen.pt` | generated by `python -m experiments.yoloe_select_prompts` | PPE visual-prompt cache |
| `mivolo_imdb.pth.tar` | [MiVOLO release](https://github.com/WildChlamydia/MiVOLO/releases) | MiVOLO demographics |
| `yolov8x_person_face.pt` | bundled with MiVOLO release | MiVOLO's internal detector |

For the first three, just running the pipeline will fetch them. The
MiVOLO pair needs to be downloaded once and dropped into
`data/weights/`. The YOLOE VPE cache needs to be generated locally —
see [`experiments/yoloe_select_prompts.py`](experiments/yoloe_select_prompts.py).

### 6. CUDA-13 loader quirk (Linux only)

If torch is on a CUDA 13 wheel (`torch-2.11+cu130`), libnvrtc may not be
on the loader path. Prepend it before running any GPU command:

```bash
export LD_LIBRARY_PATH=$(pwd)/task/lib/python3.12/site-packages/nvidia/cu13/lib:$LD_LIBRARY_PATH
```

You can put that line in `task/bin/activate` so it picks up automatically
each time you activate the venv.
